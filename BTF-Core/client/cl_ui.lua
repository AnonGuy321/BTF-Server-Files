...
RegisterCommand(
    ...
    function()
        a = true
        TriggerEvent("BTF-Client:UI:Toggle", false)
    end
)
RegisterCommand(
    "showui",
    function()
        a = false
        TriggerEvent("BTF-Client:UI:Toggle", true)
    end
)
RegisterNetEvent("BTF:ToggleHud")
AddEventHandler(
    "BTF:ToggleHud",
    function()
        a = not a
...
local l = 10.0
local m = {0, 0, 0, 255}
function tBTF.setMaxUnderWaterUITimenewTime(n)
    l = n
end
local function o(p, q, r, s, t, u, v, w)
    DrawRect(p + r / 2, q + s / 2, r, s, t, u, v, w)
end
function tBTF.getCachedMinimapAnchor()
    return k
end
function tBTF.getCachedResolution()
    ...
    function()
        while true do
            if not a then
                    ...
                    local E = tBTF.getShowHealthPercentageFlag()
                    if C > 1.0 then
                        C = 1.0
                    end
