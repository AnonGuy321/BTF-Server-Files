...
returnedMelee2 = {}
RMenu.Add('KnifeStoreMenu', 'main', RageUI.CreateMenu("", "~r~BTF Knife Store", 1300, 50, "banners", "knife"))
RMenu.Add("KnifeStoreMenu", "sub", RageUI.CreateSubMenu(RMenu:Get("KnifeStoreMenu", "main"), "", "~r~BTF Knife Store", 1300, 50, "banners", "knife"))
RMenu.Add("KnifeStoreMenu", "whietlistedmelees", RageUI.CreateSubMenu(RMenu:Get("KnifeStoreMenu", "main"), "", "~r~BTF Knife Store", 1300, 50, "banners", "knife"))
RageUI.CreateWhile(1.0, RMenu:Get('KnifeStoreMenu', 'main'), nil, function()
    RageUI.IsVisible(RMenu:Get('KnifeStoreMenu', 'main'), true, false, true, function()
        for i , p in pairs(knife.knives) do 
            RageUI.Button(p.name , "~g~"..knife.currency..tostring(getMoneyStringFormatted(p.price)), { RightLabel =  "→→→"}, true, function(Hovered, Active, Selected)
                if Selected then
        ...
        for i , o in pairs(returnedMelee2) do 
            RageUI.Button(o.name , "~g~"..knife.currency..tostring(getMoneyStringFormatted(o.price)), { RightLabel = "→→→" }, true, function(Hovered, Active, Selected)
                if Selected then
    ...
    RageUI.IsVisible(RMenu:Get("KnifeStoreMenu", "sub"), true, false, true, function()
        RageUI.Button("Purchase Weapon Body" , "Purchase "..currentMeleeName.." and Max Ammo", { RightLabel = "~g~£"..tostring(getMoneyStringFormatted(currentMeleePrice)) }, true, function(Hovered, Active, Selected)
            if Selected then
                local Ped = PlayerPedId()
                if HasPedGotWeapon(Ped, currentMeleeHash, false) then
                    notify("~r~You already have this weapon equipped.")
                else
                    TriggerServerEvent('BTF:BuyKnife', currentMeleeHash)
                end
            end
        end)
    end, function()
    ...
    RageUI.IsVisible(RMenu:Get("KnifeStoreMenu", "whietlistedmelees"), true, false, true, function()
        RageUI.Button("Purchase Weapon Body" , "Purchase "..currentMeleeName1.." and Max Ammo", { RightLabel = "~g~£"..tostring(getMoneyStringFormatted(currentMeleePrice1)) }, true, function(Hovered, Active, Selected)
            if Selected then
                local Ped = PlayerPedId()
                if HasPedGotWeapon(Ped, currentMeleeHash1, false) then
                    notify("~r~You already have this weapon equipped.")
                else
                    TriggerServerEvent('BTF:BuyWLKnife', currentMeleeHash1)
                end
            end
        end)
...
RegisterNetEvent('BTF:Error')
AddEventHandler('BTF:Error', function()
