...
tBTF.hasPoliceJob = function(bool)
    police = bool
end

tBTF.PanicCL = function(player, s1, s2, name)
    if police then
    ...
    while true do
        if showPanicBlip and police and not panic then
            ...
            if tostring(GetStreetNameFromHashKey(streetA)) and tostring(GetStreetNameFromHashKey(streetB)) then
                ...
                BTFserver.PanicSV({player,  GetStreetNameFromHashKey(streetA), GetStreetNameFromHashKey(streetB)})
                BTFserver.BlipSV({plyPos.x, plyPos.y, plyPos.z})
            else
                BTFserver.PanicSV({player,  "No street found", ""})
                BTFserver.BlipSV({plyPos.x, plyPos.y, plyPos.z})
            end
        end
