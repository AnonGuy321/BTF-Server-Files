...
RMenu.Add('LargeArmsMenu', 'main', RageUI.CreateMenu("", "~r~Large Arms", 1300, 50, "banners", "large"))
RMenu.Add("LargeArmsMenu", "sub", RageUI.CreateSubMenu(RMenu:Get("LargeArmsMenu", "main"), "", "~r~Large Arms Gun Store", 1300, 50, "banners", "large"))
RMenu.Add("LargeArmsMenu", "whitelistedguns", RageUI.CreateSubMenu(RMenu:Get("LargeArmsMenu", "main"), "", "~r~Large Arms Gun Store", 1300, 50, "banners", "large"))
...
RageUI.CreateWhile(1.0, RMenu:Get('LargeArmsMenu', 'main'), nil, function()
    ...
    RageUI.IsVisible(RMenu:Get('LargeArmsMenu', 'main'), true, false, true, function()
        RageUI.Separator('~g~You are required to have ~b~Gang', function() end)
        RageUI.Separator('~g~License to purchase guns from here.', function() end)
        for i , p in pairs(large.guns) do 
            RageUI.Button(""..p.name , '~g~'..large.currency..tostring(getMoneyStringFormatted(p.price)), {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
                if Active then
                    currentGunHash = nil
                    hoveredArmour = false
                end
        ...
        RageUI.Button(o.name , '~g~'..large.currency..tostring(getMoneyStringFormatted(o.price)), {RightLabel = '→→→'}, true, function(Hovered, Active, Selected)
            if Active then
                currentGunHash1 = nil
                hoveredArmour = false
            end
        ...
        RageUI.Button("Level 2 Armour [50%]" , "~g~£0", {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
            if Active then
                hoveredArmour = true
            end
            if Selected then
                TriggerServerEvent('BTF:BuyArmour', 50000, 50)
            end
        end)
    ...
    RageUI.IsVisible(RMenu:Get("LargeArmsMenu", "sub"), true, false, true, function()
        RageUI.Button("Purchase Weapon Body" , "Purchase "..currentGunName.." and Max Ammo", { RightLabel = "~g~£"..tostring(getMoneyStringFormatted(currentGunPrice)) }, true, function(Hovered, Active, Selected)
            if Selected then
                ...
                TriggerServerEvent("BTF:BuyGun", currentGunHash)
                end
            end
        end)
    ...
    RageUI.IsVisible(RMenu:Get("LargeArmsMenu", "whitelistedguns"), true, false, true, function()
        RageUI.Button("Purchase Weapon Body" , "Purchase  and Max Ammo", {  RightLabel = "~g~£"..tostring(getMoneyStringFormatted(currentGunPrice1)) }, true, function(Hovered, Active, Selected)
            if Selected then
                local Ped = PlayerPedId()
