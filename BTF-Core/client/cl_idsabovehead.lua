...
RegisterCommand("farids", function()
    if faridsenabled then
        TriggerServerEvent("BTF:IDsAboveHead", true)
    else
        TriggerServerEvent("BTF:IDsAboveHead", false)
    end
end)
...
RegisterCommand("hidechat", function()
    TriggerEvent('BTF:toggleChat', true)
    notify('~g~Chat Hidden')
end)

RegisterCommand("showchat", function()
    TriggerEvent('BTF:toggleChat', false)
    notify('~g~Chat Shown')
end)

RegisterNetEvent("BTF:ChangeIDs")
AddEventHandler("BTF:ChangeIDs", function(status)
    if status then
        disPlayerNames = 10
