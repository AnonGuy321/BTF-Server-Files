            ...
            if isInArea(v1, 1.4) then 
                ...
                if IsControlJustPressed(0, 51) then
                  if taxiCooldown == false then
                    TriggerEvent("BTF:PlaySound", "starter") 
                    ...
                    Citizen.CreateThread(function() 
                        ...
                        ShowNotification("~p~You should now drive to simeons to purchase your first car, a waypoint has been set for you.")
...
AddEventHandler(':cprAnim', function(nplayer)
  if GetEntityHealth(GetPlayerPed(GetPlayerFromServerId(nplayer))) == 102 then
    if not cpr_in_progress then
      ...
      Wait(12000)
      if cprChance == 1 then
        TriggerServerEvent(":SendFixClient", nplayer)
        notify("~p~You have saved this person's life.")
      else
