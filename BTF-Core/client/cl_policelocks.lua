...
RegisterNetEvent('btf:load')
AddEventHandler('btf:load', function(list)
	doors = list
end)

RegisterNetEvent('btf:statusSend')
AddEventHandler('btf:statusSend', function(i, status)
	...
	while true do
		...
		if not IsPedSittingInAnyVehicle(GetPlayerPed(-1)) then
			if IsControlJustPressed(table.unpack(LockHotkey)) then
				...
				if id ~= 0 then
					TriggerServerEvent("btf:open", id)
				end
			end
		end
	end
