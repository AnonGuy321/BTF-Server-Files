...
cfg.blackjackTables = {
    ...
    [4] = {
        dealerPos = vector3(773.63604736328,-554.79766845703,22.499017715454),
        dealerHeading = 313.0,
        tablePos = vector3(1144.429, 247.3352, -52.041),
        tableHeading = 135.31,
        distance = 1000.0,
    ...
    while true do
        if closestChair ~= -1 and closestChairDist < 2 then 
            if IsControlJustPressed(0, 38) then --Sit down [E]
                ...
                if blackjackTableData[closestChair] == false then
                    if closestChair > 7 then
                        TriggerServerEvent("Blackjack:requestSitAtBlackjackTable",closestChair) 
                    else
                        TriggerServerEvent("Blackjack:requestSitAtBlackjackTableNorm",closestChair) 
                    end
                end
            end
        end
...
RMenu.Add('BTFblackjack', 'instructions', RageUI.CreateMenu("", "test",0,100,"casinoui_cards_blackjack", "casinoui_cards_blackjack"))
RMenu:Get('BTFblackjack', 'instructions'):SetSubtitle("~g~BLACKJACK")
RMenu.Add('BTFblackjack_high', 'instructions', RageUI.CreateMenu("", "test",0,100,"casinoui_cards_blackjack_high", "casinoui_cards_blackjack_high"))
RMenu:Get('BTFblackjack_high', 'instructions'):SetSubtitle("~g~BLACKJACK")


RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('BTFblackjack', 'instructions')) then
        RageUI.DrawContent({ header = true, glare = true, instructionalButton = true }, function()           
            ...
            end, RMenu:Get('BTFblackjack', 'instructions'))            
        end, function()
            ---Panels
        end)
    end
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('BTFblackjack_high', 'instructions')) then
        RageUI.DrawContent({ header = true, glare = true, instructionalButton = true }, function()           
            RageUI.FakeButton("test", "The aim of Blackjack is to beat the Dealer's hand without going over 21. This game uses four 52           card decks, which are shuffled at the start of every hand.                                                                                    The dealer will stand on soft 17.", { RightLabel = "→→→" }, true, function(Hovered, Active, Selected)
                if (Hovered) then
            ...
            end, RMenu:Get('BTFblackjack_high', 'instructions'))            
        end, function()
            ---Panels
        end)
    end
...
function showHowToBlackjack(flag)
    if closestChair < 8 then 
        RageUI.Visible(RMenu:Get('BTFblackjack', 'instructions'), flag)
    else 
        RageUI.Visible(RMenu:Get('BTFblackjack_high', 'instructions'), flag)
    end
end
function rrrrrrrrrrround(exact, quantum)
    ...
    while true do
        if blackjackInstructional ~= nil then 
            ...
            if waitingForBetState then 
                ...
                -- DrawRect(0.944, 0.845, 0.081, 0.032, 0, 0, 0, 150)
                --DrawAdvancedNativeText(1.025, 0.853, 0.005, 0.0028, 0.25, "Chips: " .. rrrrrrrrrrround(playerAmountOfChips, 1), 255, 255, 255, 255, 0, 0)
...
function goToBlackjackSeat(blackjackSeatID)
    ...
    --print("[BTF Casino] start sit at blackjack seat") 
    ...
    --print("[BTF Casino] blackjack anims loaded") 
    ...
    --print("[BTF Casino] walking to seat, x: " .. tostring(walkToVector.x) .. " y: " .. tostring(walkToVector.y) .. " z: " .. tostring(walkToVector.z))
    ...
    --print("[BTF Casino] Blackjack sit at table net scene starting")
    --print("[BTF Casino] creating Scene at, x: " .. tostring(goToVector.x) .. " y: " .. tostring(goToVector.y) .. " z: " .. tostring(goToVector.z))
    ...
    --print("[BTF Casino] Blackjack sit at table net scene started")
...
function betChipsForNextHand(chipsAmount,chipsProp,something,chairID,someBool,zOffset)
    ...
    while not HasModelLoaded(chipsProp) do  
        Wait(0)
        --print("[BTF Casino] Stuck requesting model: " .. tostring(chipsProp))
        RequestModel(chipsProp)
    end
    vVar8 =  vector3(0.0, 0.0, getTableHeading(blackjack_func_368(chairID)))
    local tablePosX,tablePosY,tablePosZ = getTableCoords(blackjack_func_368(chairID))
...
function blackjack_func_374(betAmount, iParam1, chairId, bParam3) --returns vector3
    ...
    if not bParam3 then 
        ...
        elseif chairId == 4 then 
            if iParam1 == 0 then
                vVar1 = vector3(774.68432617188,-555.79663085938, 0.0)
            elseif iParam1 == 1 then
                vVar1 = vector3(773.92346191406,-556.49224853516, 0.0)
            elseif iParam1 == 2 then
                vVar1 = vector3(772.96960449219,-556.27349853516, 0.0)
            elseif iParam1 == 3 then
                vVar1 = vector3(772.42181396484,-555.52215576172, 0.0)
            end
        end 
    else 
        if betAmount == 10 then
