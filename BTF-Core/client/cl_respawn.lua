...
RegisterNetEvent("BTF:PolicePerms")
AddEventHandler("BTF:PolicePerms",function(pd)
    hasPD = pd
end)
RegisterNetEvent("BTF:RebelPerms")
AddEventHandler("BTF:RebelPerms",function(rebel)
    hasRebel = rebel
end)
RegisterNetEvent("BTF:VIPPerms")
AddEventHandler("BTF:VIPPerms",function(vip)
    hasVIP = vip
end)
        ...
        for a , b in pairs(respawn.hospitals) do 
            RageUI.Button(a, nil, "", true, function(Hovered, Active, Selected)
                if Selected then
                    ...
                    TriggerEvent("btf:PlaySound", "gtaloadin") 
                end
            end)
        end
        if hasVIP then
...
RegisterNetEvent("BTF:StartRespawnCam")
AddEventHandler("BTF:StartRespawnCam",function()
    inRedZone = false
    inCam = true
    inMenu = true
    DoScreenFadeIn(500)
...
RegisterNetEvent("BTF:HousingTable")
AddEventHandler("BTF:HousingTable",function(houses)
    housetable = houses
end)