RMenu.Add('BTFtruckjob', 'main', RageUI.CreateMenu("", "~g~Truck Job", nil, nil, "banners", ""))
RMenu:Get('BTFtruckjob', 'main'):SetPosition(1350, 10)
...
local truckJobPay = 0
RageUI.CreateWhile(1.0, RMenu:Get('BTFtruckjob', 'main'), nil, function()

    RageUI.IsVisible(RMenu:Get('BTFtruckjob', 'main'), true, false, true, function()
            ...
            RageUI.Button(v.name , "~g~On completion you will receive £" ..Comma(v.pay), { RightLabel = '→→→'}, true, function(Hovered, Active, Selected)
                if Selected then
                    if not jobStarted then
                        ...
                        TriggerServerEvent('BTF:startTruckJob', v.name)
                    else
            ...
            RageUI.Button("End Job" , "~r~Stop your current job", { RightLabel = '→→→'}, true, function(Hovered, Active, Selected)
                if Selected then
                    if jobStarted then
                        ...
                        TriggerServerEvent('BTF:finishTruckJob')
                        truckJobPay = 0
                    else
        ...
        if isInArea(vector3(truckjob.startLocation), 2.5) then 
            ...
            if IsControlJustPressed(0, 51) then 
                if not IsPedSittingInAnyVehicle(PlayerPedId()) then
                    RageUI.CloseAll()
                    RageUI.Visible(RMenu:Get("BTFtruckjob", "main"), true)
                else
                    ...
                    if jobStarted then
                        if (IsPedSittingInAnyVehicle(PlayerPedId())) then
                            if IsVehicleModel(GetVehiclePedIsIn(PlayerPedId(), true), GetHashKey(truckjob.vehicle)) then
                                ...
                                TriggerServerEvent('BTF:finishTruckJob', currentTruckJob)
                                jobStarted = false
                                truckJobCoords = nil
...
function SpawnTruck()
	...
	SetVehicleNumberPlateText(spawned_car, "BTF")
