...
-- Credit to Evn for making this for BTF.
...
RMenu.Add("BTF:LicenseStore", "main", RageUI.CreateMenu("", "~b~License Store", 1350, 50,"banners","licenses"))
RMenu.Add("BTF:LicenseStore", "sub", RageUI.CreateSubMenu(RMenu:Get("BTF:LicenseStore", "main"), "", "~b~License Store", 1350, 50))
RMenu.Add("BTF:LicenseStore", "confirm", RageUI.CreateSubMenu(RMenu:Get("BTF:LicenseStore", "sub"), "", "~b~Confirm License Purchase", 1350, 50))

RageUI.CreateWhile(1.0, RMenu:Get("BTF:LicenseStore", "main"), nil, function()
    RageUI.IsVisible(RMenu:Get("BTF:LicenseStore", "main"), true, false, true, function()

        RageUI.Button("~g~Grinding Licenses", nil, {RightLabel = ">>>"}, true, function(Hovered, Active, Selected)
            if Selected then
                currentType = "grinding"
            end
        end, RMenu:Get("BTF:LicenseStore", "sub", true))
    ...
    RageUI.IsVisible(RMenu:Get("BTF:LicenseStore", "sub"), true, false, true, function()
            ...
            if v.type == currentType then
                RageUI.Button(v.name, nil, {RightLabel = "~g~£"..getMoneyStringFormatted(v.price)}, true, function(Hovered, Active, Selected)
                    if Selected then
                        ...
                        RMenu:Get("BTF:LicenseStore", "confirm"):SetSubtitle("~b~Are you sure you want to buy "..v.name.."?")
                    end
                end, RMenu:Get("BTF:LicenseStore", "confirm", true))
            end
        end
    ...
    RageUI.IsVisible(RMenu:Get("BTF:LicenseStore", "confirm"), true, false, true, function()
        ...
        end, RMenu:Get("BTF:LicenseStore", "main", true))
    ...
    while true do
        ...
        if isInArea(cfg.licenseShopLocation, 1.0) and inMenu == false then
            ...
            if IsControlJustPressed(0, 51) then 
                inMenu = true
                RageUI.Visible(RMenu:Get("BTF:LicenseStore", "main"), true)
            end
        end
