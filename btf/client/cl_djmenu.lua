...
RMenu.Add('MUSICMENU','main',RageUI.CreateMenu("Sound System","~b~DJ Mixer",1300,250))
RMenu.Add('MUSICMENU','admin',RageUI.CreateSubMenu(RMenu:Get("MUSICMENU","main"),"DJ Admin","~b~DJ Admin Menu",1300,250))
...
RageUI.CreateWhile1(1.0,RMenu:Get('MUSICMENU','main'),nil,function()
    RageUI.IsVisible1(RMenu:Get('MUSICMENU','main'),true,false,true,function()
        if not a.active then 
            ...
            RageUI.Button("Help","Assistance message",{},true,function(k,l,m)
                if m then 
                    TriggerEvent("BTF:showNotification",{text="The Video ID is the ID at the end of the YouTube URL after =",height="200px",width="auto",colour="#FFF",background="#32CD32",pos="bottom-right",icon="success"},5000)
                end 
            end)
        end 
    end,function()
            ...
            for o,p in pairs(i)do 
                if i[o]~=nil then 
                    ...
                    RageUI.ButtonWithStyle("ID: "..p[3].." - Started: "..p[5].."","Name: "..p[4].." Distance Away: "..distance.." metres",{RightLabel="→→→"},true,function(k,l,m)
                        if m then 
                            TriggerServerEvent("BTF:adminStopSong",p[1])
                            table.remove(i,o)
                        end 
                    end)
                end 
...
RegisterNetEvent("BTF:playDjSong")
AddEventHandler("BTF:playDjSong",function(s,coords,t,u)
...
RegisterNetEvent("BTF:requestCurrentProgress")
AddEventHandler("BTF:requestCurrentProgress",function(s,coords)
    if not g then 
...
RegisterNetEvent("BTF:toggleDjMenu")
AddEventHandler("BTF:toggleDjMenu",function()
    RageUI.Visible(RMenu:Get('MUSICMENU','main'),true)
    h=true 
end)
RegisterNetEvent("BTF:toggleDjAdminMenu")
AddEventHandler("BTF:toggleDjAdminMenu",function(x)
    RageUI.Visible(RMenu:Get('MUSICMENU','admin'),true)
    i=x 
end)
RegisterNetEvent("BTF:finaliseSong")
AddEventHandler("BTF:finaliseSong",function(y)
    if a.active then 
        TriggerServerEvent("BTF:playDjSongServer",y,a.coords)
    end 
end)
RegisterNetEvent("BTF:updateDjVolume")
AddEventHandler("BTF:updateDjVolume",function(coords,z)
    if specificSongPlaying(coords)then 
        SendNUIMessage({type='djVolume',volume=z})
    end 
end)
RegisterNetEvent("BTF:stopSong")
AddEventHandler("BTF:stopSong",function(coords)
    local A=false
    for o,p in pairs(c)do 
        if p[1]==coords then 
            A=true
...
RegisterNetEvent("BTF:skipDj")
AddEventHandler("BTF:skipDj",function(coords,B)
    if specificSongPlaying(coords)then 
        if B then 
...
function skipDj(B)
    if songPlaying()then 
        TriggerServerEvent("BTF:skipServer",a.coords,B)
    end 
end
function setupDj()
    a.active=true
...
function stopDj()
    if songPlaying()then 
        TriggerServerEvent("BTF:stopSongServer",a.coords)
    end
    a.active=false
...
function updateVolume()
    if songPlaying()then 
        TriggerServerEvent("BTF:updateVolumeServer",a.coords,b.volume)
    end 
end
...
function func_checkDjSongs(J)
    for o,p in pairs(c)do 
        if#(J.playerCoords-p[1])>45.0 then 
            ...
            if not c[o][3]then 
                g=true
                TriggerServerEvent('BTF:requestCurrentProgressServer',o,p[1])
                c[o][3]=true 
            end 
        end 
    end 
...
RegisterNUICallback('returnProgress',function(K,L)
    if K.progress~=nil then 
        if K.progress~=0 then 
            TriggerServerEvent("BTF:returnProgressServer",d,e,K.progress)
        end 
    end
    L('return')
end)
RegisterNetEvent("BTF:returnProgress")
AddEventHandler("BTF:returnProgress",function(s,coords,M,N)
    if g then 
        if specificSongPlaying(coords)then 
            c[s]=nil
            c[s]={coords,s,true}
