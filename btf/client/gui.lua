...
-- pause
AddEventHandler("BTF:pauseChange", function(paused)
  SendNUIMessage({act="pause_change", paused=paused})
end)

-- MENU

function tBTF.openMenuData(menudata)
  SendNUIMessage({act="open_menu", menudata = menudata})
end

function tBTF.closeMenu()
  SendNUIMessage({act="close_menu"})
end

-- PROMPT

function tBTF.prompt(title,default_text)
...
function tBTF.request(id,text,time)
...
RegisterNUICallback("menu",function(data,cb)
  if data.act == "close" then
    BTFserver.closeMenu({data.id})
  elseif data.act == "valid" then
    BTFserver.validMenuChoice({data.id,data.choice,data.mod})
  end
end)
...
RegisterNUICallback("prompt",function(data,cb)
  if data.act == "close" then
    SetNuiFocus(false)
    SetNuiFocus(false)
    BTFserver.promptResult({data.result})
  end
end)
...
RegisterNUICallback("request",function(data,cb)
  if data.act == "response" then
    BTFserver.requestResult({data.id,data.ok})
  end
end)
...
function tBTF.announce(background,content)
...
-- try to fix missing cfg issue (cf: https://github.com/ImagicTheCat/BTF/issues/89)
for i=1,5 do
...
function tBTF.setProgressBar(name,anchor,text,r,g,b,value)
...
function tBTF.setProgressBarValue(name,value)
  SendNUIMessage({act="set_pbar_val", name = name, value = value})
end

-- set progress bar text
function tBTF.setProgressBarText(name,text)
  SendNUIMessage({act="set_pbar_text", name = name, text = text})
end

-- remove a progress bar
function tBTF.removeProgressBar(name)
...
Citizen.CreateThread(function()
  while true do
    ...
    -- open general menu
    if IsControlJustPressed(table.unpack(cfg.controls.phone.open)) and (not tBTF.isInComa() or not cfg.coma_disable_menu) and (not tBTF.isHandcuffed() or not cfg.handcuff_disable_menu) and GetLastInputMethod(0) then 
      BTFserver.openMainMenu({})
    end
    ...
    if pause_menu and not paused then
      paused = true
      TriggerEvent("BTF:pauseChange", paused)
