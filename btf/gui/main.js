window.addEventListener("load", function() {
    ...
    wprompt.onClose = function() {
        $.post("https://btf/prompt", JSON.stringify({ act: "close", result: wprompt.result.substring(0, 1000) }));
    };

    requestmgr.onResponse = function(id, ok) {
        $.post("https://btf/request", JSON.stringify({ act: "response", id: id, ok: ok }));
    };

    ogrpMenu.onClose = function() {
        $.post("https://btf/menu", JSON.stringify({ act: "close", id: ogrpMenu.id }));
    };

    ogrpMenu.onValid = function(choice, mod) {
        $.post("https://btf/menu", JSON.stringify({ act: "valid", id: ogrpMenu.id, choice: choice, mod: mod }));
    };
...
function djRequestProgress(){
    try {
        if (player){
            var time = player.getCurrentTime();
            $.post('http://btf/returnProgress', JSON.stringify({
                progress: time})
            );
        }
    }
