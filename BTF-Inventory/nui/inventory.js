...
$(document).ready(function() {
    ...
    $('#MoveAllBtn').click(function() {
        if (personalInv) {
            $.post(`https://${GetParentResourceName()}/MoveAllBtn`, JSON.stringify({ invType: 'Plr', itemId: $(ItemSelected).attr('id'), useAll: true }));
        } else {
            $.post(`https://${GetParentResourceName()}/MoveAllBtn`, JSON.stringify({ invType: invType, itemId: $(ItemSelected).attr('id'), useAll: true }));
        }
    })    
})
...
window.addEventListener('message', function(event) {
    ...
    if (msg.action == "InventoryDisplay" && msg.showInv) {
        ...
        $('#PersonalInventoryWeight').text(`/0kg`)
        $('#SecondInventoryWeight').text(`/0kg`)
    ...
    if (msg.action == "loadItems") {
        ...
        if (msg.CurrentKG < 15) {
            $('#PersonalInventoryWeight').css('color', 'white');
        } else if (msg.CurrentKG < 10) {
            $('#PersonalInventoryWeight').css('color', 'yellow');
