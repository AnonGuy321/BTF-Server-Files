...
local cfg = module("BTF-Core", "cfg/cfg_licenses")
...
RMenu.Add("BTF:CasinoLicenseStore", "main", RageUI.CreateMenu("", "~b~Casino License Store", 1350, 50,"banners","licenses"))
RMenu.Add("BTF:CasinoLicenseStore", "sub", RageUI.CreateSubMenu(RMenu:Get("BTF:CasinoLicenseStore", "main"), "", "~b~License Store", 1350, 50))
RMenu.Add("BTF:CasinoLicenseStore", "remove", RageUI.CreateSubMenu(RMenu:Get("BTF:CasinoLicenseStore", "main"), "", "~b~Confirm Remove License", 1350, 50))
RMenu.Add("BTF:CasinoLicenseStore", "confirm", RageUI.CreateSubMenu(RMenu:Get("BTF:CasinoLicenseStore", "sub"), "", "~b~Confirm License Purchase", 1350, 50))

RageUI.CreateWhile(1.0, RMenu:Get("BTF:CasinoLicenseStore", "main"), nil, function()
    RageUI.IsVisible(RMenu:Get("BTF:CasinoLicenseStore", "main"), true, false, true, function()

        RageUI.Button("Casino Licenses", nil, {RightLabel = ">>>"}, true, function(Hovered, Active, Selected)
            if Selected then
                currentType = "casino"
            end
        end, RMenu:Get("BTF:CasinoLicenseStore", "sub", true))

    end, function() 
    end)

    RageUI.IsVisible(RMenu:Get("BTF:CasinoLicenseStore", "sub"), true, false, true, function()
            ...
            if v.type == currentType then
                RageUI.Button(v.name, nil, {RightLabel = "~g~£"..getMoneyStringFormatted(v.price)}, true, function(Hovered, Active, Selected)
                    if Selected then
                        ...
                        RMenu:Get("BTF:CasinoLicenseStore", "confirm"):SetSubtitle("~b~Are you sure you want to buy "..v.name.."?")
                    end
                end, RMenu:Get("BTF:CasinoLicenseStore", "confirm", true))
            end
        end
        ...
        end, RMenu:Get("BTF:CasinoLicenseStore", "remove", true))

    end, function() 
    end)

    RageUI.IsVisible(RMenu:Get("BTF:CasinoLicenseStore", "confirm"), true, false, true, function()
        ...
        RageUI.Button("~g~Confirm Purchase", nil, {RightLabel = "~g~£"..getMoneyStringFormatted(currentLicensePrice)}, true, function(Hovered, Active, Selected)
            if Selected then
                TriggerServerEvent("BTF:BuyLicense", currentLicense)
            end
        end, RMenu:Get("BTF:CasinoLicenseStore", "main", true))
    ...
    RageUI.IsVisible(RMenu:Get("BTF:CasinoLicenseStore", "remove"), true, false, true, function()
        ...
        RageUI.Button("~g~Confirm Removal", nil, {RightLabel = "~g~£0"}, true, function(Hovered, Active, Selected)
            if Selected then
                TriggerServerEvent("BTF:RemoveLicense", currentLicense)
            end
        end, RMenu:Get("BTF:CasinoLicenseStore", "main", true))
    ...
    while true do
        ...
        if isInArea(cfg.casinolicenseShopLocation, 1.0) and inMenu == false then
            ...
            if IsControlJustPressed(0, 51) then 
                inMenu = true
                RageUI.Visible(RMenu:Get("BTF:CasinoLicenseStore", "main"), true)
            end
        end
