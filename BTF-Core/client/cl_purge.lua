...
RegisterCommand("purge", function(source,args,rawCommand)
    purge = not purge
    print(purge)
    TriggerServerEvent('BTF:activatePurgeSV', purge)
end)

RegisterCommand("testgun", function(source,args,rawCommand)
    if purge then
        TriggerServerEvent('BTF:purgeRespawn')
    end
end)
...
RegisterNetEvent("BTF:activatePurge")
AddEventHandler("BTF:activatePurge", function()
    purgeActivatingAnnounce = true
    TriggerEvent("btf:PlaySound", "purge")
    Wait(10000)
...
Citizen.CreateThread(function()
    while true do 
       if purgeActivatingAnnounce then
            ...
            AddTextComponentString("~r~Purge is being activated!")
            EndTextComponent()
       ...
       elseif purgeActivateAnnounce then
            PushScaleformMovieFunction(scaleform, "SHOW_SHARD_WASTED_MP_MESSAGE")
            BeginTextComponent("STRING")
            AddTextComponentString("~r~Purge is now active!")
            EndTextComponent()
            PopScaleformMovieFunctionVoid()
            DrawScaleformMovieFullscreen(scaleform, 255, 255, 255, 255)
