...
RMenu.Add('RebelMenu', 'main', RageUI.CreateMenu("", "~r~Rebel", 1300, 50, "banners", "rebel"))
RMenu.Add("RebelMenu", "sub", RageUI.CreateSubMenu(RMenu:Get("RebelMenu", "main"), "", "~r~Rebel", 1300, 50, "banners", "rebel"))
RMenu.Add("RebelMenu", "adrebel", RageUI.CreateSubMenu(RMenu:Get("RebelMenu", "main"), "", "~r~Rebel", 1300, 50, "banners", "rebel"))
RMenu.Add("RebelMenu", "armourplate", RageUI.CreateSubMenu(RMenu:Get("RebelMenu", "main"), "", "~r~Rebel", 1300, 50, "banners", "rebel"))
...
RageUI.CreateWhile(1.0, RMenu:Get('RebelMenu', 'main'), nil, function()
    RageUI.IsVisible(RMenu:Get('RebelMenu', 'main'), true, false, true, function()
        --RageUI.Separator("~r~For more guns purchase Advanced Rebel.", function() end)
        for i , p in pairs(rebel.guns) do 
            RageUI.Button(p.name , "~g~£"..tostring(getMoneyStringFormatted(p.price)) , { RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
                if Active then
                    currentGunHash = nil
                    hoveredArmour = false
                end
        ...
        if hasAids then 
            for f , a in pairs(adrebel.guns) do 
                RageUI.Button(a.name , "~g~£"..tostring(getMoneyStringFormatted(a.price)) , { RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
                    if Active then
                        currentGunHash1 = nil
                        hoveredArmour = false
                    end
        ...
        RageUI.Button(v.name , nil, { RightLabel = "~g~£"..tostring(getMoneyStringFormatted(v.price)) }, true, function(Hovered, Active, Selected)
            if Selected then
                plateprice = v.price
                plateid = v.itemID
            end
        ...
        RageUI.Button("Level 4 Armour [Max]" , nil, { RightLabel = "~g~£"..tostring(getMoneyStringFormatted(rebel.fullarmourprice)) }, true, function(Hovered, Active, Selected)
            if Active then
                hoveredArmour = true
            end
            if Selected then
    ...
    RageUI.IsVisible(RMenu:Get("RebelMenu", "armourplate"), true, false, true, function()
        RageUI.Button("Purchase Armour Plate" , nil, {   RightLabel = "~g~£"..tostring(getMoneyStringFormatted(plateprice)) }, true, function(Hovered, Active, Selected)
            if Selected then
                -- TriggerServerEvent("Rebel:BuyWeapon2", currentGunHash1)
...
RegisterNetEvent("BTF:PlaySound")
AddEventHandler("BTF:PlaySound", function(status)
    if status == 1 then 
    ...
    while true do
        ...
        for k, v in pairs(rebel.gunshops) do
            ...
            if isInArea(v, 1.4) and isInRebel == false then 
                alert('Press ~INPUT_VEH_HORN~ to access ~r~Rebel Store')
