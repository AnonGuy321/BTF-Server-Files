...
function callTakeHostage()
	...
	for i=1, #takeHostage.allowedWeapons do
		hostageHandsUp = IsEntityPlayingAnim(GetPlayerPed(GetClosestPlayer(3)), "missminuteman_1ig_2", "handsup_enter", 3)
		if hostageHandsUp then
			if GetCurrentPedWeapon(PlayerPedId(), takeHostage.allowedWeapons[i], false) then
	...
	if not canTakeHostage then 
		drawNativeNotification("~r~You need a pistol out with ammo to take a hostage at gunpoint!")
	end
...
Citizen.CreateThread(function()
	...
	while true do 
		if takeHostage.type == "agressor" then
			...
			drawNativeText("~r~Press [G] to release, [H] to kill")
