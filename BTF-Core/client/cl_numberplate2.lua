...
local location = vector3(-585.08062744141,-209.0948638916,38.219749450684)
local m = module("BTF-Veh", "cfg/cfg_garages")
m=m.garage_types

RMenu.Add('PlateShop', 'main', RageUI.CreateMenu("Number Plate", "~g~Number Plate", 1350, 50))
RMenu.Add("PlateShop", "sub", RageUI.CreateSubMenu(RMenu:Get("PlateShop", "main"), "Number Plate", "~g~Number Plate", 1350, 50))
...
RageUI.CreateWhile(1.0, RMenu:Get("PlateShop", "main"), nil, function()
    ...
    RageUI.IsVisible(RMenu:Get("PlateShop", "sub"), true, false, true, function()
        RageUI.Button("Change Number Plate", "~g~Changing plate of "..selectedCarName, {RightLabel = "~g~£50,000"}, true, function(Hovered, Active, Selected)
            if Selected then
                TriggerServerEvent("BTF:ChangeNumberPlate", selectedCar)
            end
        end)
    ...
    while true do
        ...
        if isInArea(location, 1.4) and isInMenu == false then 
            ...
            if IsControlJustPressed(0, 51) then 
                TriggerServerEvent('BTF:getCars')
...
RegisterNetEvent("BTF:RecieveNumberPlate")
AddEventHandler("BTF:RecieveNumberPlate", function(numplate)
...
RegisterNetEvent("BTF:carsTable")
AddEventHandler("BTF:carsTable",function(cars)
    carstable = cars
end)
