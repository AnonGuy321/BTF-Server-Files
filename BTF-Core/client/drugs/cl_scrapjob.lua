          ...
          while InRangeGather do
            ...
            if IsControlJustReleased(0, 51)  then
              ...
              if not pedinveh then
                ...
                BTFcfgdrugsServer.ScrapJobGather ()
                Action = false
              else
                BTF.notify({"~r~You cant gather ScrapJob while Driving!"})
              end
            end
          end
        end)
...
RMenu.Add('ScrapJobMenu', 'ScrapJob Seller', RageUI.CreateMenu("", "Scrap Job Seller",1300, 50, "banners", "scrapjob"))
...
RageUI.CreateWhile(1.0, RMenu:Get('ScrapJobMenu', 'ScrapJob Seller'):SetPosition(1300, 50), nil, function()
  RageUI.IsVisible(RMenu:Get('ScrapJobMenu', 'ScrapJob Seller'), true, false, true, function()
    ...
    RageUI.Button("Sell Scrap" , nil, {RightLabel = "~g~£1,000"}, true, function(Hovered, Active, Selected)
        if Selected then
...
function sellingScrapJobUnits()
    BTFcfgdrugsServer.SellScrapJob({tonumber(1)})
    return false
end
