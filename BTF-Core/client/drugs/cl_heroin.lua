          ...
          while InRangeGather do
            ...
            if IsControlJustReleased(0, 51)  then
              if not pedinveh then
                ...
                BTFcfgdrugsServer.HeroinGather ()
                Action = false
              else
                BTF.notify({"~r~You cant gather Heroin while Driving!"})
              end
            end
          end
        end)
        ...
        Citizen.CreateThread(function()
          while InRangeProcess do
            ...
            if IsControlJustReleased(0, 51)  then
              ...
              exports.rprogress:Start("", 5000)
              BTFcfgdrugsServer.CanProcessHeroin({}, function(canProcess, ketAmount)
                if canProcess and ketAmount then
                  ...
                  BTFcfgdrugsServer.ProcessHeroin()

                  BTF.notify({"~w~Processed~g~ Heroin~w~ 1."})
                  Action = false
                end
                if not canProcess then
                  BTF.notify({"~r~You do not have the required license."})
                    ...
                    if not ketAmount then
                      BTF.notify({"~r~You do not have enough Opium."})
                    end
                end
              end)
...
RageUI.CreateWhile(1.0, RMenu:Get('HeroinMenu', 'Heroin Seller'):SetPosition(1300, 50), nil, function()
  RageUI.IsVisible(RMenu:Get('HeroinMenu', 'Heroin Seller'), true, false, true, function()
    ...
    RageUI.Button("Sell Heroin" , nil, {RightLabel = "~g~£10,000"}, true, function(Hovered, Active, Selected)
        if Selected then
            sellingHeroinUnits()
        end
    end)
...
function sellingHeroinUnits()
    BTFcfgdrugsServer.SellHeroin({tonumber(1)})
    return false
end
