...
RegisterCommand("carry",function(source, args)
    if not carry.InProgress then
        ...
        if closestPlayer then
            ...
            if targetSrc ~= -1 then
                TriggerServerEvent("BTF:CarryRequest", targetSrc, OMioDioMode)
            else
...
RegisterNetEvent("BTFEXTRAS:StartCarry")
AddEventHandler("BTFEXTRAS:StartCarry", function(targetSrc)
...
RegisterNetEvent("BTFEXTRAS:CarryTargetAsk")
AddEventHandler("BTFEXTRAS:CarryTargetAsk", function(senderSrc)
    carryrequest = true
    Citizen.CreateThread(function()
        while carryrequest do
            if IsControlJustPressed(1, 83) then
                TriggerServerEvent("BTFEXTRAS:CarryAccepted",senderSrc)
                carryrequest = false
            elseif IsControlJustPressed(1, 84) then
                TriggerServerEvent("BTFEXTRAS:CarryDeclined",senderSrc)
                carryrequest = false
            end
            Wait(0)
        end
