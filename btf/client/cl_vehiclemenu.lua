...
Citizen.CreateThread(function()
    while true do 
        if m and getPlayerVehicle()~=k.limitingVehicle then 
            ...
            predictedMax=nil
            notify("~r~Vehicle Changed, stopping limiter")
        end
        Wait(500)
    end 
end)
...
function autoDrive()
    ...
    Citizen.CreateThread(function()
        while k.autoDrive and u do 
            ...
            u=w and not inWater
            if not w then 
                notify('~r~AutoDrive automatically disabled because the engine is inactive')
                k.autoDrive=false 
            elseif inWater then 
                notify('~r~AutoDrive automatically disabled because you are in water')
                k.autoDrive=false 
            end
            Wait(100)
        end
...
function cruise()
    ...
    Citizen.CreateThread(function()
        ...
        if collision then 
            notify('~r~Cruise control automatically disabled because a collision was detected')
        elseif not engineStatus then 
            notify('~r~Cruise control automatically disabled because the engine was turned off')
        elseif inWater then 
            notify('~r~Cruise control automatically disabled because you are in water')
        end
        n=false
        k.cruise=false 
    end)
...
RMenu.Add('vehicle_menu','locations',RageUI.CreateSubMenu(RMenu:Get('vehicle_menu','main'),"GPS","~b~LOCATIONS",nil,nil))
RageUI.CreateWhile1(1.0,RMenu:Get('vehicle_menu','main'),nil,function()
    RageUI.IsVisible1(RMenu:Get('vehicle_menu','main'),true,true,true,function()
            ...
            if J then 
                ...
                if K~=0 then 
                    if GetVehicleDoorLockStatus(K)~=2 then 
                        ...
                        notify("~w~Doors are now ~r~Locked")
                    else 
                ...
                if K~=0 then 
                    ...
                    if L==10 then
                        for i = 0, 10 do
                            ...
                            notify("~r~Opened ~w~all doors.")
                        end
                    elseif L==9 then 
                        SetVehicleDoorsShut(K,false)
                        notify("~r~Closed ~w~all doors.")
                    elseif L==8 then 
                        if r then 
                            r=false
                            RollDownWindow(K,0)
                            RollDownWindow(K,1)
                            notify("Rolled windows ~r~Down")
                        else 
                            r=true
                            RollUpWindow(K,0)
                            RollUpWindow(K,1)
                    ...
                    elseif IsVehicleDoorDamaged(K,M)then 
                        notify("~r~Cannot shut this door when the door is damaged")
                    elseif GetVehicleDoorAngleRatio(K,M)==0 then 
                        SetVehicleDoorOpen(K,M,false,false)
                        notify(string.format("The ~b~%s ~w~door is now ~g~Open",h[L]))
                    else 
                        SetVehicleDoorShut(K,M,false)
                        notify(string.format("The ~b~%s ~w~door is now ~r~Closed",h[L]))
                    end 
                end 
            end
            k.door=L 
                ...
                if k.limiter then 
                    ...
                    if not p then 
                        ...
                        if convert(speed)<10.0 then 
                            ...
                            p=false
                            notify("~r~Alert~w~: Please slow down to enable the speed limiter.")
                        end 
                    end 
                else 
            ...
            if J then 
                ...
                if GetIsVehicleEngineRunning(K)then 
                    if K~=0 then 
                        ...
                        notify("You've turned the ignition into the ~r~off ~w~position.")
                    end 
                else 
                    if K then 
    ...
    RageUI.IsVisible1(RMenu:Get('vehicle_menu','locations'),true,true,true,function()
