...
RegisterNetEvent('btf_doors:load')
AddEventHandler('btf_doors:load', function(list)
    doors = list
end)
RegisterNetEvent('btf_doors:statusSend')
AddEventHandler('btf_doors:statusSend', function(id,status)
    ...
    while true do
        ...
        for k,v in pairs(doors) do
            ...
            if GetDistanceBetweenCoords(coords.x, coords.y, coords.z, v.coords[1], v.coords[2], v.coords[3], true) <= newrange then
                ...
                if closeDoor ~= 0 then
                    if v.locked then
                        DrawText3Ds(v.coords[1], v.coords[2], v.coords[3], "🔒 Locked")
                        ...
                        DrawText3Ds(v.coords[1], v.coords[2], v.coords[3], "🔓 Unlocked")
                        FreezeEntityPosition(closeDoor, v.locked)
                    end
        ...
        if v.close ~= nil then
            ...
            if door1 < newrange then
                if door1 < GetDistanceBetweenCoords(x,y,z,doors[v.close].coords[1], doors[v.close].coords[2], doors[v.close].coords[3],true) then
                    TriggerServerEvent("btf_doors:status", k, not v.locked)
                else
                    TriggerServerEvent("btf_doors:status", v.close, not doors[v.close].locked)
                end
                break
            end
        else
