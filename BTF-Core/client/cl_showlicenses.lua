...
Citizen.CreateThread(function()
    while true do
        ...
        if IsControlJustPressed(0, 167) then
            ...
            if showMenu then
                TriggerServerEvent("BTF:RequestLicenses")
            end
        end
        if showMenu then
            DrawRect(0.50, 0.222, 0.223, 0.075, 16, 86, 229, 255)
            DrawAdvancedText(0.595, 0.217, 0.005, 0.0028, 0.7, "BTF Licenses", 255, 255, 255, 255, 6, 0)
...
RegisterNetEvent("BTF:RecieveLicenses")
AddEventHandler("BTF:RecieveLicenses", function(table)
    licenses = table
end)
