local cfg = module("BTF-Veh", "cfg/cfg_speedcap")
...
legionJD = {x = 140.92597961426, y = -1056.8748779297, z = 29.192356109619}
...
adminIsland = {x = 4577.169921875, y = -4891.3227539062, z = 16.281650543213}
...
local blips = {
	...
	{title="Safe Zone", colour=2, id=1, pos=vector3(legionJD.x, legionJD.y, legionJD.z),dist=44,nonRP=false,setBit=false},
	...
	--{title="RP Zone", colour=2, id=1, pos=vector3(MissionRow.x, MissionRow.y, MissionRow.z),dist=33,MissionRow=true,MissionRow=true},
...
if not purge then
	...
	local pos = AddBlipForRadius(333.91488647461,-597.16156005859,29.292747497559, 40.0) -- st thomas
	SetBlipColour(pos, 2)
	SetBlipAlpha(pos, 170)
	local pos = AddBlipForRadius(140.92597961426,-1056.8748779297,29.192356109619, 44.0) -- jd green 
	SetBlipColour(pos, 2)
	SetBlipAlpha(pos, 170)
	...
	SetBlipColour(pos, 2)
	SetBlipAlpha(pos, 170)
	-- local pos = AddBlipForRadius(448.98864746094,-984.71618652344,30.68931388855, 33.0) -- mission row
	-- SetBlipColour(pos, 27)
	-- SetBlipAlpha(pos, 170)
end
		...
		for index,info in pairs(blips) do
			...
			while safeZoneDist < info.dist do
				...
				if info.nonRP then
					...
					if info.MissionRow then 
						setDrawMissionRow = true
					end
...
Citizen.CreateThread(function()
	while true do
		...
		if setDrawMissionRow then
			inRedZone = false
			bank_drawTxt(0.83, 1.40, 1.0, 1.0, 0.43, "~g~You have entered a RP Zone, Do not break character", 0, 255, 0, 255)
			DisableControlAction(2, 37, true) -- disable weapon wheel (Tab)
			DisablePlayerFiring(GetPlayerPed(-1),true) -- Disables firing all together if they somehow bypass inzone Mouse Disable
			DisableControlAction(0, 45, true)
			DisableControlAction(0, 24, true)
		...
		if setDrawAdminIsland then
			inRedZone = false
			bank_drawTxt(0.83, 1.40, 1.0, 1.0, 0.43, "You have entered Admin Island, ~r~Do not leave until your sit is dealt with", 0, 255, 0, 255)
			DisableControlAction(2, 37, true) -- disable weapon wheel (Tab)
			DisablePlayerFiring(GetPlayerPed(-1),true) -- Disables firing all together if they somehow bypass inzone Mouse Disable
			DisableControlAction(0, 45, true)
			DisableControlAction(0, 24, true)
...
Citizen.CreateThread(function()
	while true do
		greenzoneTimer = greenzoneTimer - 15
		Wait(1000)
	end
end)
