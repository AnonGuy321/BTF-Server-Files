BTFCbc = {}
Tunnel.bindInterface("btf_chief",BTFCbc)
Proxy.addInterface("btf_chief",BTFCbc)
BTFSbc = Tunnel.getInterface("btf_chief","btf_chief")
...
function BTFCbc.startMissionbook()
    notputontable = true
...
function meat()
    BTF.playAnim({true, {task="PROP_HUMAN_BBQ"}, false})
    BTF.notify({"~w~You now preparing ~g~Meat"})
    Wait(30000)
    BTF.stopAnim({false})
    BTF.notify({"~w~Finishng to preparing ~g~Meat"})
    BTFSbc.getfood({1})
end

function fries()
    BTF.playAnim({true, {task="PROP_HUMAN_BBQ"}, false})
    BTF.notify({"~w~You now preparing ~g~Fries"})
    Wait(30000)
    BTF.stopAnim({false})
    BTF.notify({"~w~Finishng to preparing ~g~Fries"})
    BTFSbc.getfood({2})
end

function fruitsandvegetabels()
    BTF.playAnim({true, {task="PROP_HUMAN_PARKING_METER"}, false})
    BTF.notify({"~w~You now take ~g~Fruit and Vegetables"})
    Wait(3000)
    BTF.stopAnim({false})
    BTFSbc.getfood({3})
end
...
function coke()
    BTF.playAnim({true, {task="PROP_HUMAN_PARKING_METER"}, false})
    BTF.notify({"~w~You take a ~g~Coke"})
    Wait(3000)
    BTF.stopAnim({false})
    BTFSbc.getfood({4})
end
