$(document).ready(function(){
  ...
  $('input').change(function(){
    $.post('http://BTFbarber/updateSkin', JSON.stringify({
      value: false,
      // Face
  ...
  $(document).keypress(function(e) {
    if(e.which == 113){ // A pressed
      $.post('http://BTFbarber/rotaterightheading', JSON.stringify({
        value: 10
      }));
    }
    if(e.which == 101){ // E pressed
      $.post('http://BTFbarber/rotateleftheading', JSON.stringify({
        value: 10
      }));
    }
    if(e.which == 119 ){ // W pressed
      $.post('http://BTFbarber/zoomin', JSON.stringify({
        value: 10
      }));
    }
    if(e.which == 115){ // S pressed
      $.post('http://BTFbarber/zoomout', JSON.stringify({
        value: 10
      }));
    }
  });
  ...
  $('#tabs label').on('click', function(e){
    //e.preventDefault();
    $.post('http://BTFbarber/zoom', JSON.stringify({
  ...
  $(document).keypress(function(e){
    ...
    xTriggered++;
    if(e.which == 13){
      $.post('http://BTFbarber/test', JSON.stringify({
        value: xTriggered
      }));
    }
  });
...
function leftWristComponent() {
    $.post('http://BTFbarber/updateLeftWristComponent', JSON.stringify({
...
function leftWristTexture() {
    ...
    $.post('http://BTFbarber/updateLeftWristTexture', JSON.stringify({
        componentID: $('.montre .active').attr('data'),
...
function rightWristComponent() {
    $.post('http://BTFbarber/updateRightWristComponent', JSON.stringify({
...
function rightWristTexture() {
    ...
    $.post('http://BTFbarber/updateRightWristTexture', JSON.stringify({
        componentID: $('.montre-right .active').attr('data'),
