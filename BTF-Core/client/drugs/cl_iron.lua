        ...
        Citizen.CreateThread(function()
          while InRangeProcess do
            ...
            if IsControlJustReleased(0, 51)  then
              ...
              exports.rprogress:Start("", 5000)
              BTFcfgdrugsServer.IronCanProcess({}, function(canProcess, ketAmount)
                if canProcess and ketAmount then
                  ...
                  BTFcfgdrugsServer.IronDoneProcessing()
                  BTF.notify({"~w~Processed~g~ Iron~w~ 1."})
                  Action = false
                ...
                if not canProcess then
                  BTF.notify({"~r~You do not have the required license."})
                    ...
                    if not ketAmount then
                      BTF.notify({"~r~You do not have enough Iron Ore."})
                    end
                end
              end)
...
RageUI.CreateWhile(1.0, RMenu:Get('IronMenu', 'Iron Seller'):SetPosition(1300, 50), nil, function()
  RageUI.IsVisible(RMenu:Get('IronMenu', 'Iron Seller'), true, false, true, function()
      RageUI.Button("Sell Iron" , nil, {RightLabel = "~g~£1,500"}, true, function(Hovered, Active, Selected)
...
function sellingIronUnits()
     BTFcfgdrugsServer.SellIronJob({tonumber(1)})
    return false
end
