...
Citizen.CreateThread(function()
    TriggerServerEvent("BTF:PoliceCheckRadial")
    TriggerServerEvent("BTF:NHSCheckRadial")
end)
...
function Crosshair(enable)
    TriggerEvent("BTF:PutCrossHairOnScreen", enable)
    Display = enable;
end
...
CreateThread(function()
    ...
    while true do
        ...
        if EntityType then
            ...
            if Display and GetVehiclePedIsIn(playerPed,false) ~= 0 then
                Crosshair(false)
            end
...
RegisterNetEvent("BTF:CleanCar")
AddEventHandler("BTF:CleanCar",function()
    if GetEntityHealth(GetPlayerPed(-1)) >= 103 and not IsEntityDead(GetPlayerPed(-1)) then 
        ...
        SetNuiFocus(false, false)
        TaskStartScenarioInPlace(GetPlayerPed(-1), 'world_human_maid_clean', 0, 1)
        Wait(10000)
        SetVehicleDirtLevel(GetVehiclePedIsIn(GetPlayerPed(-1),true), 0.0)
        SetVehicleUndriveable(GetVehiclePedIsIn(GetPlayerPed(-1),true), false)
