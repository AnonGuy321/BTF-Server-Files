...
RMenu.Add('CinematicMenu', 'MainMenu', RageUI.CreateMenu("", "BTF Cinematic", 1300,100,"settings","settings")) 
    ...
    if RageUI.Visible(RMenu:Get('CinematicMenu', 'MainMenu')) then
        ...
        if HasCinematic then
            ...
            RageUI.Button("Get Permission", nil, {RightLabel = "→→"}, true, function(Hovered, Active, Selected)
                if Selected then
                    SendNUIMessage({act="openurl",url="https://discord.gg/btf5m"})
                end
            end)
        end
       end)
