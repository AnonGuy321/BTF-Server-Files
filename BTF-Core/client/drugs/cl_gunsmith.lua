          ...
          while InRangeGather do
            ...
            if IsControlJustReleased(0, 51)  then
              ...
              TaskStartScenarioInPlace(ped, 'WORLD_HUMAN_GARDENER_PLANT', false, true)
                ...
                BTFcfgdrugsServer.DiamondGather()
                Action = false
              else
        ...
        Citizen.CreateThread(function()
          while InRangeProcess do
            ...
            if IsControlJustReleased(0, 51)  then
              ...
              exports.rprogress:Start("", 5000)
              BTFcfgdrugsServer.DiamondCanProcess({}, function(canProcess, ketAmount)
                if canProcess and ketAmount then
                  ...
                  BTFcfgdrugsServer.DiamondDoneProcessing()
                  BTF.notify({"~w~Processed~g~ Diamond~w~ 1."})
                  Action = false
                ...
                if not canProcess then
                  BTF.notify({"~r~You do not have the required license."})
                    ...
                    if not ketAmount then
                      BTF.notify({"~r~You do not have enough Diamond Crystals."})
                    end
                end
              end)
...
RageUI.CreateWhile(1.0, RMenu:Get('DiamondMenu', 'Diamond Seller'):SetPosition(1300, 50), nil, function()
  RageUI.IsVisible(RMenu:Get('DiamondMenu', 'Diamond Seller'), true, false, true, function()
      RageUI.Button("Sell Diamond" , nil, {RightLabel = "~g~£10,000"}, true, function(Hovered, Active, Selected)
...
function sellingDiamondUnits()
     BTFcfgdrugsServer.SellDiamondJob({tonumber(1)})
    return false
end
