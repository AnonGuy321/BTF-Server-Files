...
local m = module("BTF-Veh", "cfg/cfg_garages")
m=m.garage_types
...
RageUI.CreateWhile(1.0, true, function()
        ...
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            if b then
                RageUI.Button("Spawn Vehicle (No mods)", "", {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
                    if Selected then
                        AddTextEntry('FMMC_MPM_NC', "Enter the car spawncode name ~r~[If you spell it wrong relog]")
                ...
                RageUI.Button("~r~Delete Vehicle", "Deletes your vehicle", {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
                    if Selected then
                ...
                RageUI.Button("Cycle through seats" , nil, { RightLabel = '→→→'}, true, function(Hovered, Active, Selected) 
                    if Selected then 
                        ...
                        if IsPedInAnyVehicle(p) then
                            ...
                            tBTF.notify("~r~Not in a vehicle.")
                        end
                    end
                end)
                ...
                if b then
                    RageUI.Checkbox("Return to normal Universe", "~r~Leave Car Dev Mode", b, {}, function(Hovered, Active, Selected, Checked)
                        if Selected then
                            ...
                            b=false
                            TriggerServerEvent("BTF:setCarDev",b)
                        end
                    end)
                else
    ...
    if RageUI.Visible(RMenu:Get('CARDEV', 'vehiclelist')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            RageUI.Separator("~r~Not all cars in this list work")
            if b then
...
RegisterCommand('cardev', function()
    TriggerServerEvent('BTF:openCarDev')
end)

function tBTF.openCarDev()
...
function TakeVehScreenshot()
    ...
    ped = GetVehiclePedIsIn(PlayerPedId(), true)
    SetEntityCoords(ped, vector3(-75.357818603516,-818.87200927734,326.17532348633))
    SetEntityHeading(PlayerPedId(),100.0)
    SetGameplayCamRelativePitch(7.5,1.0)
    SetGameplayCamRelativeHeading(-180.0)
    Wait(3000)
    TriggerServerEvent('BTF:takeCarScreenshot')
    Wait(5000)
    ...
    notify("~g~Successfully Sent all screenshots via ~y~Discord.")
end
local A={
...
local function a1()
    ...
    TriggerServerEvent('BTF:getCarDevDebug',{a2})
end
...
local function a8()
    ...
    if CursorInArea(0.25,0.31,0.12,0.15)and IsDisabledControlJustPressed(0,24)then 
        a1()
        TriggerEvent("btf:PlaySound", "apple") 
        notify("~g~Successfully Sent handling via discord ~y~look in the handle channel.")
    end
    ...
    if CursorInArea(0.31,0.37,0.12,0.15)and IsDisabledControlJustPressed(0,24)then 
        ...
        notify("~g~Successfully Reset vehicles stats!")
    end 
end
Citizen.CreateThread(function()
    while true do
