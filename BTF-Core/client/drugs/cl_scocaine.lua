          ...
          while InRangeGather do
            ...
            if IsControlJustReleased(0, 51)  then
              ...
              TaskStartScenarioInPlace(ped, 'WORLD_HUMAN_GARDENER_PLANT', false, true)
                ...
                BTFcfgdrugsServer.CocaineGather()
                Action = false
              else
                BTF.notify({"~r~How the fuck are you supposed to gather sitting in that thing?"})
              end
            end
          end
        end)
        ...
        Citizen.CreateThread(function()
          while InRangeProcess do
            ...
            if IsControlJustReleased(0, 51)  then
              ...
              exports.rprogress:Start("", 5000)
              BTFcfgdrugsServer.CocaineCanProcess({}, function(canProcess, ketAmount)
                if canProcess and ketAmount then
                  ...
                  BTFcfgdrugsServer.CocaineDoneProcessing()
                  BTF.notify({"~w~Processed~g~ Cocaine~w~ 1."})
                  Action = false
                ...
                if not canProcess then
                  BTF.notify({"~r~You do not have the required license."})
                    ...
                    if not ketAmount then
                      BTF.notify({"~r~You do not have enough Cocaine."})
                    end
                end
              end)
...
Citizen.CreateThread(function() 
    while true do
            ...
            if isInArea(v1, 1.4) then 
                alert('Press ~INPUT_VEH_HORN~ to access ~r~Cocaine Seller')
...
RageUI.CreateWhile(1.0, RMenu:Get('CocaineMenu', 'Cocaine Seller'):SetPosition(1300, 50), nil, function()
  RageUI.IsVisible(RMenu:Get('CocaineMenu', 'Cocaine Seller'), true, false, true, function()
      RageUI.Button("Sell Cocaine" , nil, {RightLabel = "~g~£1,500"}, true, function(Hovered, Active, Selected)
...
function sellingCocaineUnits()
     BTFcfgdrugsServer.SellCocaineJob({tonumber(1)})
    return false
end
