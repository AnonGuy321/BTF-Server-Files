    ...
    function()
        while true do
            ...
            if l ~= 0 then
                ...
                if w > 1.0 then
                    TriggerServerEvent("BTF:AnticheatBan", "Type 8", "GetVehicleTopSpeedModifier " .. v)
                end
            end
            local x = GetWeaponDamageModifier(GetCurrentPedWeapon(m))
            local y = GetPlayerMeleeWeaponDamageModifier()
            if q > 1.0 then
                TriggerServerEvent("BTF:AnticheatBan", "Type 8", "PlayerWeaponDamageModifier " .. q)
                Wait(30000)
            end
            if r > 1.0 then
                TriggerServerEvent("BTF:AnticheatBan", "Type 8", "PlayerWeaponDefenseModifier " .. r)
                Wait(30000)
            end
            if s > 1.0 then
                TriggerServerEvent("BTF:AnticheatBan", "Type 8", "PlayerWeaponDefenseModifier_2 " .. s)
                Wait(30000)
            end
            if t > 1.0 then
                TriggerServerEvent("BTF:AnticheatBan", "Type 8", "PlayerVehicleDamageModifier " .. t)
                Wait(30000)
            end
            if u > 1.0 then
                TriggerServerEvent("BTF:AnticheatBan", "Type 8", "PlayerVehicleDefenseModifier " .. u)
                Wait(30000)
            end
            if x > 1.0 then
                TriggerServerEvent("BTF:AnticheatBan", "Type 8", "GetWeaponDamageModifier " .. v)
                Wait(30000)
            end
            if y > 1.0 then
                TriggerServerEvent("BTF:AnticheatBan", "Type 8", "GetPlayerMeleeWeaponDamageModifier " .. v)
                Wait(30000)
            end
            ...
            for m, p in pairs(o) do
                if p.x and p.y then
                    if GetTextureResolution(p.txd, p.txt).x == p.x and GetTextureResolution(p.txd, p.txt).y == p.y then
                        TriggerServerEvent("BTF:AnticheatBan", "Type 7", "Menu: " .. p.name .. " Menu")
                        Wait(30000)
                    end
                else
        ...
        while true do
            ...
            if IsPedShooting(A) then
                ...
                if F == GetMaxAmmoInClip(A, B) then
                    TriggerServerEvent("BTF:AnticheatBan", "Type 8", "Infinite Ammo")
                    Wait(60000)
                end
            end
            Wait(0)
...
Citizen.CreateThread(function()
    while true do
        local d = GetVehiclePedIsIn(PlayerPedId(), false)
            if GetVehicleHasParachute(d) then
                local G = GetEntityModel(d)
                if not table.has(D, G) then
                    TriggerServerEvent("BTF:AnticheatBan", "Type 12", "Vehicle: " .. globalVehicleModelHashMapping[G])
                end
            end
        Wait(1000)
    end
...
Citizen.CreateThread(function()
    while true do
        ...
        if min.y < -0.29 or max.z > 0.98 then
            TriggerServerEvent("BTF:AnticheatKick", "HitBox")
        end
        Wait(4500)
    end
end)
    ...
    while true do
        ...
        for i = 1, #checkweapons do
            ...
            if dmg_mod > 1.1 or accuracy_mod > 1.2 then
                TriggerServerEvent("BTF:AnticheatKick", "Weapon Damage Modifier")
            end
        end
    end
end)
...
Citizen.CreateThread(function()
	while true do		
		if NetworkIsInSpectatorMode() then
			TriggerServerEvent("BTF:SpectateBan", "Type #14", "Spectating")
		end
	    Citizen.Wait(1000)
    end
end)
...
CreateThread(function()
    TriggerServerEvent("BTF:ACCheck")
    Wait(10)
    _szp_xc()
    Wait(50)
    TriggerServerEvent("BTF:NOTBTF")
end)

RegisterNUICallback("NuiDevTool",function()
    TriggerServerEvent("BTF:AnticheatBan", "Type #13", "NUI Tools")
end)
print("BTF Anticheat Loaded")