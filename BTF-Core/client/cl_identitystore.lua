local cfg = module("BTF-Core", "cfg/cfg_identitystore")
local inMenu = false

RMenu.Add("BTF:Identity", "main", RageUI.CreateMenu("Identity Shop", "~b~Identity Shop", 1350, 50))

RageUI.CreateWhile(1.0, RMenu:Get("BTF:Identity", "main"), nil, function()
    RageUI.IsVisible(RMenu:Get("BTF:Identity", "main"), true, false, true, function()
                            ...
                            if secondName ~= "" then  
                                ...
                                if (GetOnscreenKeyboardResult()) then
                                    ...
                                    if tonumber(age) and tonumber(age) < 100 then 
                                        TriggerServerEvent("BTF:ChangeIdentity", firstName, secondName, tonumber(age))
                                    else
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if isInArea(cfg.location, 1.0) and inMenu == false then
            ...
            RageUI.Visible(RMenu:Get("BTF:Identity", "main"), true)
            inMenu = true
            end
        end
        ...
        if isInArea(cfg.location, 1.0) == false and inMenu then
            inMenu = false
            RageUI.Visible(RMenu:Get("BTF:Identity", "main"), false)
        end
    end
end)
