...
RegisterNetEvent('BTF:isPoliceVar')
AddEventHandler('BTF:isPoliceVar', function(target)

end)

RegisterNetEvent('BTF:getTackled')
AddEventHandler('BTF:getTackled', function(target)
...
RegisterNetEvent('BTF:playTackle')
AddEventHandler('BTF:playTackle', function()
	isTackling = true
	local playerPed =PlayerPedId()
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlPressed(0, Keys['LEFTSHIFT'], true) and IsControlPressed(0, Keys['G'], true) and not isTackling then
			Citizen.Wait(10)
			...
			if distance ~= -1 and distance <= 3.0 and not isTackling and not isGettingTackled and not IsPedInAnyVehicle(GetPlayerPed(-1)) and not IsPedInAnyVehicle(GetPlayerPed(closestPlayer)) then
				...
				TriggerServerEvent('BTF:tryTackle', closestPlayerId)
			end
		end
	end
end)
