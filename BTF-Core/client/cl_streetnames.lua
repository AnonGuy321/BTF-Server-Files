...
RegisterNetEvent("BTF:StreetNames")
AddEventHandler("BTF:StreetNames", function(bool)
    streetson = bool
	if streetson then
        --ShowNotification("~y~Street Names Are Now ~g~Enabled")
    else
        --ShowNotification("~y~Street Names Are Now ~r~Disabled")
    end
end)
	...
	while true do
		if streetson then
			...
			if streetson == true then 
				...
				AddTextComponentString("~w~".. hash1 .. " | ~b~" ..street2)
				DrawText(0.015, 0.75)
			end
		end
		Citizen.Wait(0);
