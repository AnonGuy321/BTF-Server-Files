...
function module(rsc, path) -- load a LUA resource file as module
    if path == nil then -- shortcut for btf, can omit the resource parameter
        path = rsc
        rsc = "btf"
    end
    local key = rsc .. path
    ...
    if modules[key] then -- cached module
        ...
        if f then
            ...
            if ar[1] then
                ...
                modules[key] = nil
                print("[BTF] error loading module " .. rsc .. "/" .. path .. ":" .. ar[2])
            end
        else
            print("[BTF] error parsing module " .. rsc .. "/" .. path .. ":" .. err)
        end
    end
end
    ...
    local finish = function(params)
        if not r.done then
            ...
            if callback then
                ...
                print("[BTF] error: callback function is nil")
            end
        end
    end
