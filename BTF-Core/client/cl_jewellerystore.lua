    ...
    while true do
        ...
        if robbing == false then
            if nextrobbing == true then
                DrawMarker(27, -631.1552734375,-229.40916442871,38.057052612305-0.95, 0, 0, 0, 0, 0, 0, 0.9, 0.9, 0.9, 255, 0, 0, 200, 0, 0, 0, 1)
                if position_verf(playerLoc.x, playerLoc.y, playerLoc.z, -631.1552734375,-229.40916442871,38.057052612305-0.95, 2) then
                    help_message("Press ~INPUT_CONTEXT~ to start the jewellery robbery.")
...
AddEventHandler('jewelryrobberry:start', function()
    notifypicture_message("CHAR_DETONATEPHONE",2,"Jewellery Store",false,"~r~The robbery has begun! Steal everything you can!")
    notifypicture_message("CHAR_DETONATEPHONE",2,"Jewellery Store",false,"~r~You have 30 seconds to steal everything before the police alarm goes off!")
    robbing = true
    nextrobbing = false
    if cancelled == false then
        SetTimeout(30000, function()
            if cancelled == false then
                TriggerEvent('btf:PlaySound', 'alarm')
                notifypicture_message("CHAR_DETONATEPHONE",1,"Jewellery Store",false,"~r~Can you hear it? The alarm went off the police were warned of the robbery!")
        ...
        if robbing == true then
            if nextrobbing == false then
                if position_verf(playerLoc.x, playerLoc.y, playerLoc.z, -632.86700439453,-238.65907287598,38.07377243042-0.95, 2.5) then
                    notifypicture_message("CHAR_DETONATEPHONE",1,"Jewellery Store",false,"~r~The robbery was cancelled as the robbers left the area.")
                    cancelled = true
                    robbing = false
                    SetTimeout(3600000, function()
                        robbing = false
...
Citizen.CreateThread(function()
    while true do
        ...
        if status1 == true and status2 == true and status3 == true and status4 == true and status5 == true and status6 == true and status7 == true and status8 == true and status8 == true and status9 == true and status10 == true and status11 == true and status12 == true and status13 == true and status14 == true and status15 == true and status16 == true and status17 == true and status18 == true and status19 == true and status20 == true and status21 == true and status22 == true and status23 == true and status24 == true and status25 == true then
            if check == true then
                notifypicture_message("CHAR_DETONATEPHONE",2,"Jewellery Store",false,"~r~You have acquired all the jewels.")
                check = false
            end
            if position_verf(playerLoc.x, playerLoc.y, playerLoc.z, -630.17553710938,-236.52540588379,38.0570602416992-0.95, 3) then
                help_message("~r~You can do the next robbery after 60 minutes from the previous one.")
            end
            SetTimeout(3600000, function()
                robbing = false
                nextrobbing = true
...
Citizen.CreateThread(function()
    ...
    AddTextComponentString("Jewellery Store")
...
RegisterNetEvent('BTF:setJewelleryBlip')
AddEventHandler('BTF:setJewelleryBlip', function(player)

    local mugshot, mugshotStr = GetPedMugshot(GetPlayerPed(GetPlayerFromServerId(player)))

    ShowAdvancedNotification('Jewellery Store Robbery', '', 'Respond immediately.', mugshotStr, 4)
    UnregisterPedheadshot(mugshot)

    local Blip = AddBlipForRadius(-622.24560546875, -230.86483764648, 38.057060241699, 100.0)
