...
Citizen.CreateThread(function()
    while true do
        ...
        if noclip then
            ...
            local dx, dy, dz = tBTF.getCamDirection()
        ...
        if EntityCleanupGun then 
            ...
            if GetSelectedPedWeapon(PlayerPedId()) == GetHashKey('WEAPON_STAFFGUN') then
                if IsPlayerFreeAiming(plr) then 
                    ...
                    if yes then 
                        tBTF.notify('~g~Deleted Entity: ' .. GetEntityModel(entity))
...
local function teleportToWaypoint()
    Citizen.CreateThread(function()
        ...
        if success then
            ...
            DoScreenFadeIn(250)
            tBTF.notify('~g~Teleported success!')
        end
    end)
end
...
RegisterNetEvent("BTF:vehicleMenu")
AddEventHandler("BTF:vehicleMenu",function(DioMode, isInTicket)
    if getStaffLevel() > 0 then
	OMioDioMode = DioMode
    ticketStatus = isInTicket
    TriggerEvent('greenzoneStaffMode', OMioDioMode)
	if not OMioDioMode then
		tBTF.setCustomization(adminTicketSavedCustomization)
        setRedzoneTimerDisabled(false)
        ...
        adminTicketSavedCustomization = tBTF.getCustomization()
        gender = getModelGender()
        if gender == "male" then
            z="mp_m_freemode_01"
            local A=loadModel(z)
            setRedzoneTimerDisabled(true)
            tBTF.setCustomization({modelhash=A})
            Wait(100)
            ...
            -- SetPedComponentVariation(B,11,200,5,00) -- [Jacket]
        ...
        elseif gender == "female" then
            ...
            SetPedComponentVariation(ped,11,63,0,00) -- [Jacket]
...
RegisterCommand("fix", function()
    if OMioDioMode or tBTF.getUserId() == 2 or tBTF.getUserId() == 1 then
...
RegisterNetEvent("BTF:showBlips")
AddEventHandler("BTF:showBlips",function()
    blips = not blips
    if blips then
        tBTF.notify("~g~Blips enabled")
    else
        tBTF.notify("~r~Blips disabled")
