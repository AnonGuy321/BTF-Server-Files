...
RMenu.Add('PDClockOn', 'main', RageUI.CreateMenu("", "~b~PD Clockon", 1300, 50, "banners", "police"))
    ...
    RageUI.IsVisible(RMenu:Get('PDClockOn', 'main'), true, false, true, function()
        for i , p in pairs(clockon) do 
            RageUI.ButtonWithStyle(p.group, nil, { RightLabel = "→→→" }, true, function(Hovered, Active, Selected)
                if Selected then
                    TriggerServerEvent('BTF:Clockon', p.group)
                    TriggerEvent('BTF:policeRemove')
                    TriggerServerEvent("BTF:PoliceCheckRadial")
                end
            end)
        end

        RageUI.ButtonWithStyle("~r~[Unemployed]", nil, { RightLabel = "→→→" }, true, function(Hovered, Active, Selected)
            if Selected then
...
RegisterNetEvent('BTF:policeRemove')
AddEventHandler('BTF:policeRemove', function()
    local source = source
